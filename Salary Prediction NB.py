# -*- coding: utf-8 -*-
"""
Created on Sun Feb 28 18:36:06 2021
...
salary_train = pd.read_csv("D:\\Data Science study\\Assignment of Data Science\\Sent\\12 Naive Bayes\\SalaryData_Train.csv")

#Importing the test data
salary_test = pd.read_csv("D:\\Data Science study\\Assignment of Data Science\\Sent\\12 Naive Bayes\\SalaryData_Test.csv")
...
# Importing Naive Bayes functions from sklearn library
...
#Now we will build the models and predict them at the same time
# But mostly we will be focused on the gaussianNB which is much more suitable for the numerical data.

pred_gnb = gsnb.fit(model_data_train,output_data_train).predict(model_data_test)
pred_mnb = mnnb.fit(model_data_train,output_data_train).predict(model_data_test)

# Lets plot the confusion matrix for gaussian_NB model
from sklearn.metrics import confusion_matrix

confusion_matrix(output_data_test,pred_gnb)
pd.crosstab(output_data_test.values.flatten(),(pred_gnb))
np.mean(pred_gnb==output_data_test.values.flatten())    # 79.8406374501992% accuracy

confusion_matrix(output_data_test,pred_mnb)
pd.crosstab(output_data_test.values.flatten(),(pred_mnb))
np.mean(pred_mnb==output_data_test.values.flatten())    # 77.49003984063745% accuracy

# you can see that the accuracy hs been dropped in the multinomial NB and hence it is more efficient to 
#use the gaussianNB for the numerical data.


#We can apply other various transformations on the data given and try to get the better results by using various functions but we 
#are mainly focused on the Naive Bayes classification technique for this task so this will be all for this case.